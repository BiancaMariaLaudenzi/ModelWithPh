...
double gasTransport::calculate_SO2(double pO2, double pCO2, double pH) {
     ...
     */
	double T_C = 37; 		// [°C] blood temperature in Celsius
	double xHbf = 0; 		// [-] fraction of fetal hemoglobin concentration in the blood
	...
	double kPa = 1;			// conversione

    double a = -0.72 * pH - 7.4										 // (27)
               + 0.09 * log10(pCO2 / (5.33 * kPa))
...
double gasTransport::calculate_cCO2(double pCO2, double pH, double SO2) {
    ...
    double pKPla = 6.125 - log10(1.0 + pow(10.0, pH - 8.7));						// (37)
...
void gasTransport::getAlgebraicRelations() {
	...
	tie(AlveoliLungsAlg[3], AlveoliLungsAlg[4]) = invertedDissociation_old(stateVars[4], stateVars[5]); //, HpulCap, AlveoliLungsAlg[3], AlveoliLungsAlg[4]); // pppo2,pppco2
	...
	if (denom != 0) {
		...
		AlveoliLungsAlg[5] = 0.0;  // mLgas/mLblood
		AlveoliLungsAlg[6] = 0.0;  // mLgas/mLblood
	}
	...
	tie(AlveoliLungsAlg[7], AlveoliLungsAlg[8]) = invertedDissociation_old(AlveoliLungsAlg[5], AlveoliLungsAlg[6]); //, HsysArt, AlveoliLungsAlg[7], AlveoliLungsAlg[8]); //pao2,paco2
